    ...
    for j in range(num_traces):
        for r in range(max(start_round, 1), end_round + 1):
            ...
            for i in range(BlockSize):
                # Fixed: we calculate the hamming weight based on the stateround, of S(p[i]^k[i])^p[i]^k[i]
...
def guess_key_hd(plaintexts, hamming_distances, r):
    ...
    for i in range(BlockSize):
        ...
        # Fixed: using the guess as the one relevant byte in the key, we encrypt and check
        # that the hamming weight is equal to the hamming distance calculated by our power analysis
        example_key = np.zeros(BlockSize, dtype=np.uint8)
        for guess in range(2**8):
            example_key[i] = guess
            aes = AESr(bytes(example_key), 2)
            for j in range(num_traces):
                if plaintexts[j]:
                    stateround = aes.encrypt_r(plaintexts[j], r, 0)
                    if hamming_distances[j][r][i] != hamming_weight(AES.S[stateround[i]]^stateround[i]):
...
def guess_k1_hd(plaintexts, hamming_distances, k0):
    ...
    # Fixed: we generate the "plaintexts" for the second round by running a single round of aes with the guessed k0
    for j in range(num_traces):
...
def recover_full_key(key_len, k0_list, k1_list):
    ...
    # Fixed: we iterate the available keys options for k0 and k1, and create the combinations list
