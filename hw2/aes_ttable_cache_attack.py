...
def simulate_cache_access(plaintexts, key, start_round, end_round):
    ...
    for j in range(num_traces):
        for r in range(max(start_round, 1), end_round + 1):
            # in round r, we access T0[stateround[0]], T1[stateround[5]], etc.
...
def calc_ttables(l, plaintexts, partial_k0_high):
    ...
    for j in range(num_traces):
        for nibble in range(2 ** 4):
            # For each ttable, calculate the output from the AES definition for Col[l]
            T_result[j][nibble][0] = AES.T1[nibble ^ partial_k0_high[0] ^ plaintexts[j][(l*NumTables) % BlockSize]]
            T_result[j][nibble][1] = AES.T2[nibble ^ partial_k0_high[1] ^ plaintexts[j][(l*NumTables + 5) % BlockSize]]
            T_result[j][nibble][2] = AES.T3[nibble ^ partial_k0_high[2] ^ plaintexts[j][(l*NumTables + 10) % BlockSize]]
            T_result[j][nibble][3] = AES.T4[nibble ^ partial_k0_high[3] ^ plaintexts[j][(l*NumTables + 15) % BlockSize]]
...
def find_unviable_candidates(j, T_result, k0_low, viable):
    ...
    for i in range(NumTables):
        for k1_nibble in range(2 ** 4):
            # Check viablity using the AES definition of the index to T_i
            if not viable[k1_nibble ^ col_nibbles[3-i]][i]:
...
def reduce_candidates(T_result, viable, candidates, j, k0_low):
    unviable = find_unviable_candidates(j, T_result, k0_low, viable)
    for k1_nibble, i in unviable:
        candidates[k0_low][k1_nibble][i] = False
...
if __name__ == "__main__":
    ...
    # unaccessed_list = find_unaccessed(accessed_list)
    # k1_list = guess_k1_ttable(plaintexts, unaccessed_list, keyarr[:16], True)
    # print(k1_list)
